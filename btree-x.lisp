...
Return the function used by the tree for accessing keys from objects.")
...
Return the lock for the BTree")
   ...
   ( coerce-to-object (btree node-or-object) => object
...
(defun half-capacity (node)
  (let ((v (truncate (node-capacity node) 2)))
    (if (oddp v)
        (1+ v)
      v)))
  ...
  (with-locked-btree (btree)
    ...
    (um:when-let (root (root-node btree))
        (um:nlet srch ((node root))
          (um:bind*
              ((:values (found-it ix) (search-node item-key btree node))
               (cell-width (cell-width node))
               (nx         (* ix cell-width)))
            (if found-it
                (let ((pobj (node-object btree node ix)))
                  (funcall delete-fn pobj)
                  (decf (items-count btree))
                  (delete-from-node btree node nx)
                  (return-from delete-item pobj))
              ...
              (if (> cell-width 1)
                ...
                ;; else - in leaf but record not found
...
(defun delete-from-node (btree node ix)
  (um:bind*
    ...
    (if (= cell-width 1)
        ;; simple deletion
        (let ((new-limit (1- limit)))
          (if (< ix new-limit)
              (copy-node-list-cells node ix node (1+ ix) (- new-limit ix)))
          (if (= 0 new-limit)
              ;; can only happen in root node
              (setf (root-node btree) nil)
            ;; else
            (progn
              (setf (node-fill-pointer node) new-limit)
              (< new-limit (half-capacity node))) ;; t if needs coalesce
            ))
      
      ;; else - in non-leaf node -- replace obj with rightmost subtree object
      (labels ((grab-rightmost (sub-node)
                 (let* ((last-cell  (1- (node-fill-pointer sub-node)))
                        (cell-width (cell-width sub-node))
                        (hi-obj     (node-list-cell sub-node last-cell)))
                   
                   (if (= cell-width 1)
                       (progn
                         (setf (node-fill-pointer sub-node) last-cell)
                         (if (< last-cell (half-capacity sub-node))
                             (values hi-obj t) ;; t for needs coalesce
                           hi-obj))
                     
                     ;; non-leaf node -- keep looking
                     (multiple-value-bind (obj needs-coalesce)
                         (grab-rightmost hi-obj)
                       (if needs-coalesce
                           (values obj
        ...
        (multiple-value-bind (obj needs-coalesce)
            (grab-rightmost (node-list-cell node ix))
          (setf (node-list-cell node (1+ ix)) obj)
          (if needs-coalesce
              (adjust-subnodes-from-deletion btree node ix))
          ))) ))
...
(defun adjust-subnodes-from-deletion (btree node ix)
  ;; on entry pnode is the node which had a deletion
  ;; and pnode is a non-leaf node. Index nx points somewhere (anywhere)
  ;; in the node-list.
  ;;
  ;;   ... p k p k pl kk pr k p ... -- we are concerned about pl kk and pr
  ;;
  ;; on entry index nx points to pl
  ;;
  (unless (or (and (> ix 0)
                   (try-rebalancing-nodes node (- ix 2)))
              (let ((limit (node-fill-pointer node)))
